...
namespace Recog.PTests.FPI
{
    public class FPITestLoader
    {
         private FPIAnswers _answers;
            ...
            [XmlElement(Type = typeof(FPIAnswer))]
           
            public FPIAnswers Answers
            {
                get { return _answers; }
            }

            public FPITestLoader(pBaseEntities GlobalEntities, TwoButtonsControl Control)
            {
                _ge = GlobalEntities;
                _ktc = Control;
                _answers = new FPIAnswers(_ge);
                arg = new TestDoneEventArgs();
            }

        public void Start()
        {
            testsparam t = _ge.testsparams.First(tp => tp.idt == (int)EnumPTests.FPI);
        ...
        public void Stop()
        {
            arg.Reason = "Окончание тестирования";
        OnTestDone();
        }

        public void Next()
        {
            if (_currentaparam.num != 114)
            {
                testsparam t = _ge.testsparams.First(tp => tp.idt == (int)EnumPTests.FPI);
                t.answersparams.Load();
