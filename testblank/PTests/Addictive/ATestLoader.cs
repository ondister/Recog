...
namespace Recog.PTests.Addictive
{
    public class ATestLoader
    {
         private AAnswers _answers;
            ...
            [XmlElement(Type = typeof(AAnswer))]
           
            public AAnswers Answers
            {
                get { return _answers; }
            }

            public ATestLoader(pBaseEntities GlobalEntities, TwoButtonsControl Control)
            {
                _ge = GlobalEntities;
                _ktc = Control;
                _answers = new AAnswers(_ge);
                arg = new TestDoneEventArgs();
            }

        public void Start()
        {
            testsparam t = _ge.testsparams.First(tp => tp.idt == (int)EnumPTests.Addictive);
