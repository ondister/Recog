...
namespace Recog.PTests.D
{
    public class DTestLoader
    {
         private DAnswers _answers;
            ...
            [XmlElement(Type = typeof(DAnswer))]
           
            public DAnswers Answers
            {
                get { return _answers; }
            }

            public DTestLoader(pBaseEntities GlobalEntities, TwoButtonsControl Control)
            {
                _ge = GlobalEntities;
                _ktc = Control;
                _answers = new DAnswers(_ge);
                arg = new TestDoneEventArgs();
            }

        public void Start()
        {
            testsparam t = _ge.testsparams.First(tp => tp.idt == (int)EnumPTests.Adaptability);
