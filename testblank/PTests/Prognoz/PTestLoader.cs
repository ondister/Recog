...
namespace Recog.PTests.Prognoz
{
    public class PTestLoader
    {
         private PAnswers _answers;
            ...
            [XmlElement(Type = typeof(PAnswer))]
           
            public PAnswers Answers
            {
                get { return _answers; }
            }

            public PTestLoader(pBaseEntities GlobalEntities, TwoButtonsControl Control)
            {
                _ge = GlobalEntities;
                _ktc = Control;
                _answers = new PAnswers(_ge);
                arg = new TestDoneEventArgs();
            }

        public void Start()
        {
            testsparam t = _ge.testsparams.First(tp => tp.idt == (int)EnumPTests.Prognoz);
